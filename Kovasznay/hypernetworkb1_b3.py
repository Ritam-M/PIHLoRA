...
class HyperNetwork(nn.Module):
    ...
    def loss_func(self):
        ...
        for i,re in enumerate(self.train_Res):
            ...
            loss_train += (loss_u_bc1 + loss_v_bc1 + loss_p_bc1)/len(self.train_Res)
            loss_train_bc += (loss_u_bc1 + loss_v_bc1 + loss_p_bc1)/len(self.train_Res)
                        
            a = 20-i%20
            train_loss += a*(loss_u_bc1 + loss_v_bc1 + loss_p_bc1)/len(self.train_Res)
        ...
        for i,re in enumerate(self.val_Res):
            ...
            loss_val += (loss_u_bc1 + loss_v_bc1 + loss_p_bc1)/len(self.val_Res)
            loss_val_bc += (loss_u_bc1 + loss_v_bc1 + loss_p_bc1)/len(self.val_Res)
            
            a = 20-i%20
            val_loss += a*(loss_u_bc1 + loss_v_bc1 + loss_p_bc1)/len(self.val_Res)
    ...
    def predict(self,X):
        ...
        for i, re in enumerate(self.train_Res):            
            ...
            u_domain, v_domain, p_domain = self.net_u(param,x_grid,y_grid)    
